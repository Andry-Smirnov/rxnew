...
type
  ...
  public
    function Execute(AGrid: TRxDBGrid; var FilterStr: string;
...
begin
  ...
  begin
    if Assigned(AGrid.Columns[i].Field) and (AGrid.Columns[i].Field.FieldKind = fkData) and
      (AGrid.Columns[i].Visible) then
    begin
          ...
          if Edit_1[X].Items.IndexOf(Edit_1[X].Text) > -1 then
            ...
            if Pos('NULL', Combo_2[X].Text) > 0 then
              ...
              case C.Field.DataType of
                ftDateTime,
                ftDate: FilterStr :=
                    FilterStr + '(' + C.FieldName + Combo_2[X].Text + char(39) + Copy(Edit_1[X].Text, 7, 4) +
                    Copy(Edit_1[X].Text, 3, 4) + Copy(Edit_1[X].Text, 1, 2) + Copy(Edit_1[X].Text, 11, 9) +
                    char(39) + ') ';
                ftUnknown: FilterStr :=
                    FilterStr + '(' + C.FieldName + Combo_2[X].Text + Edit_1[X].Text + ') ';
                ftTime,
                ftString,
                ftMemo: FilterStr :=
                    FilterStr + '(' + C.FieldName + Combo_2[X].Text + QuotedString(Edit_1[X].Text, '''') + ') ';
                else
                  FilterStr := FilterStr + '(' + C.FieldName + Combo_2[X].Text + Edit_1[X].Text + ') ';
              end;
          LastFilter.Add(C.FieldName + '|||' + Combo_2[X].Text + '|||' +
            Edit_1[X].Text + '|||' + Combo_3[X].Text);
        end;
      end;
    end;
  end;
